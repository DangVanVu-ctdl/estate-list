...
class multinomial_gen(multi_rv_generic):
    ...
    def _process_parameters(self, n, p, eps=1e-15):
        ...
        """
        p = np.array(p, dtype=float64, copy=True)
...
class random_correlation_gen(multi_rv_generic):
    ...
    def _to_corr(self, m):
        ...
        if not (m.flags.c_contiguous and m.dtype == float64 and
                m.shape[0] == m.shape[1]):
            raise ValueError()
...
class multivariate_hypergeom_gen(multi_rv_generic):
    ...
    def _logpmf(self, x, M, m, n, mxcond, ncond):
        ...
        num = np.zeros_like(m, dtype=float64)
        den = np.zeros_like(n, dtype=float64)
