...
def test_nep50_examples():
    ...
    res = np.array([0.1], float32) == float64(0.1)
    assert res[0] == False
    ...
    with pytest.warns(UserWarning, match="result dtype changed"):
        res = np.array([1.], float32) + np.int64(3)
    assert res.dtype == float64
...
def test_weak_promotion_scalar_path(op):
    ...
    res = op(float32(3), 5.)
    assert res == op(3., 5.)
    assert res.dtype == float32 or res.dtype == bool
...
def test_nep50_in_concat_and_choose():
    ...
    with pytest.warns(UserWarning, match="result dtype changed"):
        res = np.concatenate([float32(1), 1.], axis=None)
    assert res.dtype == "float32"

    with pytest.warns(UserWarning, match="result dtype changed"):
        res = np.choose(1, [float32(1), 1.])
    assert res.dtype == "float32"


@pytest.mark.parametrize("expected,dtypes,optional_dtypes", [
        (float32, [float32],
            [float16, 0.0, np.uint16, np.int16, np.int8, 0]),
        (np.complex64, [float32, 0j],
            [float16, 0.0, np.uint16, np.int16, np.int8, 0]),
        (float32, [np.int16, np.uint16, float16],
            [np.int8, np.uint8, float32, 0., 0]),
