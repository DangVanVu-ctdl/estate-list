...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Index(np.arange(5), dtype=float64), Index(np.arange(5), dtype=np.int64)),
        (Index(np.arange(5), dtype=float64), RangeIndex(5)),
        (Index(np.arange(5), dtype=float64), Index(np.arange(5), dtype=np.uint64)),
    ],
)
...
def test_union_int_categorical_with_nan():
    ...
    result = idx.union(ci)
    expected = Index([1, 2, np.nan], dtype=float64)
    tm.assert_index_equal(result, expected)
