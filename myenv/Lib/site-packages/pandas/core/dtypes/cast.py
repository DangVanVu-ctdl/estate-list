...
def maybe_downcast_numeric(
    ...
    if dtype.kind in "biu":
        ...
        if not isinstance(element, (np.integer, floating, int, float, bool)):
        ...
        issubclass(dtype.type, floating)
...
def maybe_upcast_numeric_to_64bit(arr: NumpyIndexT) -> NumpyIndexT:
    ...
    elif dtype.kind == "f" and dtype != float64:
...
def ensure_dtype_can_hold_na(dtype: DtypeObj) -> DtypeObj:
    ...
    if isinstance(dtype, ExtensionDtype):
        if dtype._can_hold_na:
            ...
            return IntervalDtype(float64, closed=dtype.closed)
        return _dtype_obj
    elif dtype.kind == "b":
        return _dtype_obj
...
def _maybe_promote(dtype: np.dtype, fill_value=np.nan):
    ...
    if issubclass(dtype.type, np.datetime64):
        ...
        if issubclass(dtype.type, np.bool_):
            ...
            if mst > dtype:
                # e.g. mst is float64 and dtype is float32
                dtype = mst
        ...
        elif issubclass(dtype.type, (np.integer, floating)):
            mst = np.min_scalar_type(fill_value)
            dtype = np.promote_types(dtype, mst)
