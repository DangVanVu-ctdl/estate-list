...
class TestConcatAppendCommon:
    ...
    def test_concat_categorical(self):
        ...
        exp = Series([10, 11, np.nan, np.nan, 1, 3, 2], dtype=float64)
        tm.assert_series_equal(pd.concat([s1, s2], ignore_index=True), exp)
        tm.assert_series_equal(s1._append(s2, ignore_index=True), exp)
    ...
    def test_concat_categorical_coercion(self):
        ...
        exp = Series([1, 2, np.nan, 2, 1, 2], dtype=float64)
        tm.assert_series_equal(pd.concat([s1, s2], ignore_index=True), exp)
        tm.assert_series_equal(s1._append(s2, ignore_index=True), exp)

        # result shouldn't be affected by 1st elem dtype
        exp = Series([2, 1, 2, 1, 2, np.nan], dtype=float64)
        ...
        exp = Series([10, 11, np.nan, 1, 3, 2], dtype=float64)
        tm.assert_series_equal(pd.concat([s1, s2], ignore_index=True), exp)
        tm.assert_series_equal(s1._append(s2, ignore_index=True), exp)

        exp = Series([1, 3, 2, 10, 11, np.nan], dtype=float64)
        tm.assert_series_equal(pd.concat([s2, s1], ignore_index=True), exp)
        tm.assert_series_equal(s2._append(s1, ignore_index=True), exp)
    ...
    def test_concat_categorical_coercion_nan(self):
        ...
        # some edge cases
        # category + not-category => not category
        s1 = Series(np.array([np.nan, np.nan], dtype=float64), dtype="category")
        s2 = Series([np.nan, 1])
