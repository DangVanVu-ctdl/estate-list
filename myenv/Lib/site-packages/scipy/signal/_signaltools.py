...
def medfilt(volume, kernel_size=None):
    ...
    if not (np.issubdtype(volume.dtype, np.integer) 
            or volume.dtype in [float32, float64]):
...
def medfilt2d(input, kernel_size=3):
    ...
    if image.dtype.type not in (np.ubyte, float32, float64):
