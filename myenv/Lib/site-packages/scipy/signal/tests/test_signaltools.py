...
class TestMedFilt:
    ...
    @pytest.mark.parametrize('dtype', [np.bool_, np.complex64, np.complex128,
                                       np.clongdouble, float16, np.object_,
                                       "float96", "float128"])
    ...
    def test_odd_strides(self):
        ...
        dummy = np.arange(10, dtype=float64)
        a = dummy[5:6]
        a.strides = 16
        assert_(signal.medfilt(a, 1) == 5.)

    @pytest.mark.parametrize("dtype", [np.ubyte, float32, float64])
...
class TestResample:
    ...
    def test_output_float32(self, padtype):
        ...
        h = np.array([1, 1, 1], dtype=float32)
    ...
    def test_poly_vs_filtfilt(self):
        ...
        try_types = (int, float32, np.complex64, float, complex)
        size = 10000
...
class TestCorrelateReal:
    ...
    def equal_tolerance_fft(self, res_dt):
        ...
        if res_dt == np.longdouble:
            return self.equal_tolerance(float64)
        else:
...
class TestDecimate:
    ...
    def test_long_float32(self):
        ...
        x = signal.decimate(np.ones(10_000, dtype=float32), 10)
        assert not any(np.isnan(x))

    def test_float16_upcast(self):
        # float16 must be upcast to float64
        x = signal.decimate(np.ones(100, dtype=float16), 10)
        assert x.dtype.type == float64
