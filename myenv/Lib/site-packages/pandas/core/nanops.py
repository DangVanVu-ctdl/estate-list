...
def _get_empty_reduction_result(
    ...
    ret = np.empty(shp[dims != axis], dtype=float64)
    ret.fill(np.nan)
    return ret
...
def nanvar(
    ...
    *,
    ...
    avg = _ensure_numeric(values.sum(axis=axis, dtype=float64)) / count
    if axis is not None:
        avg = np.expand_dims(avg, axis)
    sqr = _ensure_numeric((avg - values) ** 2)
    if mask is not None:
        np.putmask(sqr, mask, 0)
    result = sqr.sum(axis=axis, dtype=float64) / d

    # Return variance as float64 (the datatype used in the accumulator),
...
def nanargmax(
    ...
    *,
    ...
    >>> arr = np.array(range(12), dtype=float64).reshape(4, 3)
    >>> arr[2:, 2] = np.nan
    >>> arr
    array([[ 0.,  1.,  2.],
           [ 3.,  4.,  5.],
...
def nanskew(
    ...
    *,
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        mean = values.sum(axis, dtype=float64) / count
    if axis is not None:
    ...
    m2 = adjusted2.sum(axis, dtype=float64)
    m3 = adjusted3.sum(axis, dtype=float64)
...
def nankurt(
    ...
    *,
    ...
    m2 = adjusted2.sum(axis, dtype=float64)
    m4 = adjusted4.sum(axis, dtype=float64)
...
def _get_counts(
    ...
    mask: npt.NDArray[np.bool_] | None,
    axis: AxisInt | None,
    dtype: np.dtype[floating] = np.dtype(float64),
) -> floating | npt.NDArray[floating]:
    """
