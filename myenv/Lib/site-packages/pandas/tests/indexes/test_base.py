...
class TestIndex:
    ...
    def test_constructor_int_dtype_nan(self):
        # see gh-15187
        data = [np.nan]
        expected = Index(data, dtype=float64)
    ...
    def test_empty_fancy_raises(self, index):
        ...
        empty_farr = np.array([], dtype=float64)
        empty_index = type(index)([], dtype=index.dtype)
    ...
    def test_reindex_no_type_preserve_target_empty_mi(self):
        ...
        result = index.reindex(
            MultiIndex([Index([], np.int64), Index([], float64)], [[], []])
        )[0]
        assert result.levels[0].dtype.type == np.int64
        assert result.levels[1].dtype.type == float64
...
@pytest.mark.parametrize("dtype", [None, np.int64, np.uint64, float64])
