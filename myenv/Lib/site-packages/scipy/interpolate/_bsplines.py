...
def _get_dtype(dtype):
    """Return np.complex128 for complex dtypes, float64 otherwise."""
...
class BSpline:
    ...
    **Implementation details**
    ...
    def __init__(self, t, c, k, extrapolate=True, axis=0):
        ...
        self.c = np.asarray(c)
        self.t = np.ascontiguousarray(t, dtype=float64)
    ...
    def __call__(self, x, nu=0, extrapolate=None):
        ...
        x = np.ascontiguousarray(x.ravel(), dtype=float64)
    ...
    def integrate(self, a, b, extrapolate=None):
        ...
        if extrapolate == 'periodic':
            ...
            if n_periods > 0:
                ...
                x = np.asarray([ts, te], dtype=float64)
            ...
            if b <= te:
                x = np.asarray([a, b], dtype=float64)
                _bspl.evaluate_spline(ta, ca.reshape(ca.shape[0], -1),
                                      ka, x, 0, False, out)
                integral += out[1] - out[0]
            else:
                x = np.asarray([a, te], dtype=float64)
                _bspl.evaluate_spline(ta, ca.reshape(ca.shape[0], -1),
                                      ka, x, 0, False, out)
                integral += out[1] - out[0]

                x = np.asarray([ts, ts + b - te], dtype=float64)
                _bspl.evaluate_spline(ta, ca.reshape(ca.shape[0], -1),
                                      ka, x, 0, False, out)
                integral += out[1] - out[0]
        else:
...
def _make_periodic_spline(x, y, t, k, axis):
    ...
    # kl = ku = k
    ab = np.zeros((3 * k + 1, nt), dtype=float64, order='F')
...
def make_interp_spline(x, y, k=3, t=None, bc_type=None, axis=0,
    ...
    kl = ku = k
    ab = np.zeros((2*kl + ku + 1, nt), dtype=float64, order='F')
...
def make_lsq_spline(x, y, t, k=3, w=None, axis=0, check_finite=True):
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result is
    ...
    lower = True
    extradim = prod(y.shape[1:])
    ab = np.zeros((k+1, n), dtype=float64, order='F')
