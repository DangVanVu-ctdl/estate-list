...
class TestFinfo:
    def test_basic(self):
        dts = list(zip(['f2', 'f4', 'f8', 'c8', 'c16'],
                       [float16, float32, float64, np.complex64,
                        np.complex128]))
        for dt1, dt2 in dts:
    ...
    def test_regression_gh23108(self):
        # float32(1.0) and float64(1.0) have the same hash and are
        # equal under the == operator
        f1 = np.finfo(float32(1.0))
        f2 = np.finfo(float64(1.0))
        assert f1 != f2
...
class TestRepr:
    ...
    def test_finfo_repr(self):
        ...
        assert_equal(repr(np.finfo(float32)), expected)
...
def test_instances():
    ...
    for c in [float, float16, float32, float64]:
        class_finfo = finfo(c)
...
def test_known_types():
    ...
    for ftype, ma_like in ((float16, _float_ma[16]),
                           (float32, _float_ma[32]),
                           (float64, _float_ma[64])):
