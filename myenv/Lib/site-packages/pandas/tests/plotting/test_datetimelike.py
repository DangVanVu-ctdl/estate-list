...
class TestTSPlot:
    ...
    def test_tsplot(self):
        ts = Series(
            np.arange(10, dtype=float64), index=date_range("2020-01-01", periods=10)
        )
        _, ax = mpl.pyplot.subplots()
    ...
    def test_gaps(self):
        ts = Series(
            np.arange(30, dtype=float64), index=date_range("2020-01-01", periods=30)
        )
        ts.iloc[5:25] = np.nan
        _, ax = mpl.pyplot.subplots()
        ts.plot(ax=ax)
    ...
    def test_mixed_freq_irregular_first(self):
        s1 = Series(
            np.arange(20, dtype=float64), index=date_range("2020-01-01", periods=20)
        )
        s2 = s1.iloc[[0, 5, 10, 11, 12, 13, 14, 15]]
        _, ax = mpl.pyplot.subplots()
    ...
    def test_from_weekly_resampling(self):
        ...
        expected_h = idxh.to_period().asi8.astype(float64)
        expected_l = np.array(
    ...
    def test_from_resampling_area_line_mixed(self, kind1, kind2):
        ...
        expected_y = np.zeros(len(expected_x), dtype=float64)
        for i in range(3):
            line = ax.lines[i]
        ...
        expected_x = idxh.to_period().asi8.astype(float64)
        expected_y = np.zeros(len(expected_x), dtype=float64)
        for i in range(3):
            line = ax.lines[3 + i]
            assert PeriodIndex(data=line.get_xdata()).freq == idxh.freq
    ...
    def test_mpl_nopandas(self):
        ...
        exp = np.array([x.toordinal() for x in dates], dtype=float64)
