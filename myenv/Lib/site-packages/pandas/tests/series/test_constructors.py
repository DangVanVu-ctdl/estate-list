...
class TestSeriesConstructors:
    ...
    def test_constructor_pass_nan_nat(self):
        # GH 13467
        exp = Series([np.nan, np.nan], dtype=float64)
        assert exp.dtype == float64
    ...
    @pytest.mark.parametrize("arr_dtype", [np.int64, float64])
    ...
    def test_constructor_dict(self):
        ...
        expected = Series(np.nan, pidx, dtype=float64)
        expected.iloc[0] = 0
        expected.iloc[1] = 1
        tm.assert_series_equal(result, expected)
    ...
    def test_fromValue(self, datetime_series, using_infer_string):
        nans = Series(np.nan, index=datetime_series.index, dtype=float64)
        assert nans.dtype == float64
