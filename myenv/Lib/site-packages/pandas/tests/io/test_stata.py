...
class TestStata:
        ...
        empty_df_typed = DataFrame(
            {
                ...
                "f32": np.array([0], dtype=float32),
                "f64": np.array([0], dtype=float64),
            }
        )
    ...
    def test_read_dta1(self, file, datapath):
        ...
        expected["float_miss"] = expected["float_miss"].astype(float32)
    ...
    def test_read_dta18(self, datapath):
        ...
        expected["Floats"] = expected["Floats"].astype(float32)
    ...
    def test_read_write_dta13(self):
        ...
        formatted = original
        formatted["int64"] = formatted["int64"].astype(float64)
    ...
    def test_read_write_reread_dta15(self, file, datapath):
        ...
        expected["float_"] = expected["float_"].astype(float32)
        expected["double_"] = expected["double_"].astype(float64)
    ...
    def test_nan_to_missing_value(self, version):
        s1 = Series(np.arange(4.0), dtype=float32)
        s2 = Series(np.arange(4.0), dtype=float64)
        s1[::2] = np.nan
        s2[1::2] = np.nan
    ...
    def test_string_no_dates(self):
        ...
        s2 = Series([1.0, 2.0], dtype=float64)
        original = DataFrame({"s1": s1, "s2": s2})
        original.index.name = "index"
        with tm.ensure_clean() as path:
            original.to_stata(path)
    ...
    def test_large_value_conversion(self):
        ...
        modified["s3"] = Series(modified["s3"], dtype=float64)
    ...
    def test_105(self, datapath):
        ...
        df0["psch_dis"] = df0["psch_dis"].astype(float32)
    ...
    def test_categorical_order(self, file, datapath):
        ...
        for is_cat, col, labels, codes in expected:
            if is_cat:
                ...
                cols.append((col, Series(labels, dtype=float32)))
...
"""
    ...
    def test_out_of_range_float(self):
        ...
        original.index.name = "index"
        for col in original:
            original[col] = original[col].astype(float32)

        with tm.ensure_clean() as path:
            original.to_stata(path)
            reread = read_stata(path)

        original["ColumnTooBig"] = original["ColumnTooBig"].astype(float64)
        expected = original.copy()
        expected.index = expected.index.astype(np.int32)
    ...
    def test_writer_117(self):
        ...
        original["float32"] = Series(original["float32"], dtype=float32)
        original.index.name = "index"
        original.index = original.index.astype(np.int32)
        copy = original.copy()
        with tm.ensure_clean() as path:
...
def test_precision_loss():
    ...
    with tm.ensure_clean() as path:
        with tm.assert_produces_warning(
            ...
            df.to_stata(path, write_index=False)
        reread = read_stata(path)
        expected_dt = Series([float64, float64], index=["big", "little"])
