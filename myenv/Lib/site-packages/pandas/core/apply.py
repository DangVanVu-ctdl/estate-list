...
class FrameApply(NDFrameApply):
    ...
    def apply_empty_result(self):
        ...
        if not should_reduce:
            try:
                if self.axis == 0:
                    r = self.func(
                        Series([], dtype=float64), *self.args, **self.kwargs
                    )
                else:
                    r = self.func(
                        Series(index=self.columns, dtype=float64),
                        *self.args,
                        **self.kwargs,
                    )
            except Exception:
        ...
        if should_reduce:
            if len(self.agg_axis):
                r = self.func(Series([], dtype=float64), *self.args, **self.kwargs)
            else:
                r = np.nan
    ...
    def wrap_results(self, results: ResType, res_index: Index) -> DataFrame | Series:
        ...
        if len(results) == 0 and constructor_sliced is Series:
            result = constructor_sliced(results, dtype=float64)
        else:
