...
class TestDataFrameConcat:
    def test_concat_multiple_frames_dtypes(self):
        # GH#2759
        df1 = DataFrame(data=np.ones((10, 2)), columns=["foo", "bar"], dtype=float64)
        df2 = DataFrame(data=np.ones((10, 2)), dtype=float32)
    ...
    def test_concat_copies(self, axis, order, ignore_index, using_copy_on_write):
        ...
        df = DataFrame(np.zeros((10, 5), dtype=float32, order=order))
