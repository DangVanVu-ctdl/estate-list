...
def test_masked_dtype():
    ...
    # upcast the arrays to float64. After gh16662, check expected promotion
    ...
    a = np.ma.array([1, 2, 3], mask=[0, 1, 0], dtype=float32)
    assert stats.gmean(a).dtype == float32
...
@pytest.mark.parametrize('dtype', [np.int16, float32, np.complex128])
