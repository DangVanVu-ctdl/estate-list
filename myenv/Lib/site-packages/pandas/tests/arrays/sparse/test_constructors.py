...
class TestConstructors:
    def test_constructor_dtype(self):
        ...
        assert arr.dtype == SparseDtype(float64, np.nan)
        assert arr.dtype.subtype == float64
        assert np.isnan(arr.fill_value)

        arr = SparseArray([np.nan, 1, 2, np.nan], fill_value=0)
        assert arr.dtype == SparseDtype(float64, 0)
        assert arr.fill_value == 0

        arr = SparseArray([0, 1, 2, 4], dtype=float64)
        assert arr.dtype == SparseDtype(float64, np.nan)
        assert np.isnan(arr.fill_value)
    ...
    def test_constructor_float32(self):
        # GH#10648
        data = np.array([1.0, np.nan, 3], dtype=float32)
        arr = SparseArray(data, dtype=float32)

        assert arr.dtype == SparseDtype(float32)
        tm.assert_numpy_array_equal(arr.sp_values, np.array([1, 3], dtype=float32))
        # Behavior change: np.asarray densifies.
        # tm.assert_numpy_array_equal(arr.sp_values, np.asarray(arr))
