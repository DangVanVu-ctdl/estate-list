...
def scalar_instances(times=True, extended_precision=True, user_dtype=True):
    ...
    # Floats:
    yield param(np.sqrt(float16(5)), id="float16")
    yield param(np.sqrt(float32(5)), id="float32")
    yield param(np.sqrt(float64(5)), id="float64")
...
class TestTimeScalars:
    @pytest.mark.parametrize("dtype", [np.int64, float32])
