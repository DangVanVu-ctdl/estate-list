...
def _eps_for_method(x0_dtype, f0_dtype, method):
    ...
    -------
    EPS: float
        relative step size. May be float16, float32, float64

    Notes
    -----
    The default relative step will be float64. However, if x0 or f0 are
    smaller floating point types (float16, float32), then the smallest
...
def _compute_absolute_step(rel_step, x0, f0, method):
    ...
    Notes
    -----
    `h` will always be float64. However, if `x0` or `f0` are
    smaller floating point dtypes (e.g. float32), then the absolute
