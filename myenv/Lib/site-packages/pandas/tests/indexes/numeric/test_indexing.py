...
class TestGetLoc:
    ...
    def test_get_loc_float64(self):
        idx = Index([0.0, 1.0, 2.0], dtype=float64)
    ...
    def test_get_loc_na(self):
        idx = Index([np.nan, 1, 2], dtype=float64)
        assert idx.get_loc(1) == 1
        assert idx.get_loc(np.nan) == 0

        idx = Index([np.nan, 1, np.nan], dtype=float64)
        assert idx.get_loc(1) == 1
        ...
        idx = Index([np.nan, 1, np.nan, np.nan], dtype=float64)
        assert idx.get_loc(1) == 1
        msg = "'Cannot get left slice bound for non-unique label: nan"
        with pytest.raises(KeyError, match=msg):
            idx.slice_locs(np.nan)
...
class TestGetIndexer:
    ...
    def test_get_indexer_nan(self):
        # GH#7820
        result = Index([1, 2, np.nan], dtype=float64).get_indexer([np.nan])
...
class TestWhere:
    ...
    def test_where(self, listlike_box, index):
        ...
        expected = Index([index._na_value] + index[1:].tolist(), dtype=float64)
        result = index.where(listlike_box(cond))
...
class TestTake:
    @pytest.mark.parametrize("idx_dtype", [float64, np.int64, np.uint64])
    ...
    def test_take_fill_value_float64(self):
        # GH 12631
        idx = Index([1.0, 2.0, 3.0], name="xxx", dtype=float64)
        result = idx.take(np.array([1, 0, -1]))
        expected = Index([2.0, 1.0, 3.0], dtype=float64, name="xxx")
        tm.assert_index_equal(result, expected)

        # fill_value
        result = idx.take(np.array([1, 0, -1]), fill_value=True)
        expected = Index([2.0, 1.0, np.nan], dtype=float64, name="xxx")
        tm.assert_index_equal(result, expected)
...
class TestContains:
    @pytest.mark.parametrize("dtype", [float64, np.int64, np.uint64])
    ...
    def test_contains_float64_nans(self):
        index = Index([1.0, 2.0, np.nan], dtype=float64)
