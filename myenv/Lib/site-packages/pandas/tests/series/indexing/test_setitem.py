...
class TestSetitemSlices:
    ...
    def test_setitem_slicestep(self):
        ...
        series = Series(
            np.arange(20, dtype=float64), index=np.arange(20, dtype=np.int64)
        )
...
class TestSetitemCastingEquivalents(SetitemCastingEquivalents):
    @pytest.fixture(params=[np.nan, float64("NaN"), None, NA])
    def val(self, request):
        """
        NA values that should generally be valid_na for *all* dtypes.

        Include both python float NaN and float64; only float64 has a
        `dtype` attribute.
        """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            np.array([2**65, 2**65 + 1], dtype=float64),
            FutureWarning,
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                ...
                reason="float32(1.1) ends up as 1.100000023841858, so "
                "np_can_hold_element raises and we cast to float64",
            ),
        ),
        (1 + 1j, np.complex128, FutureWarning),
        (True, object, FutureWarning),
        (np.uint8(2), float32, None),
        (np.uint32(2), float32, None),
        # float32 cannot hold np.iinfo(np.uint32).max exactly
        # (closest it can hold is 4294967300.0 which off by 5.0), so
        # we cast to float64
        (np.uint32(np.iinfo(np.uint32).max), float64, FutureWarning),
        (np.uint64(2), float32, None),
        (np.int64(2), float32, None),
    ],
)
class TestCoercionFloat32(CoercionTest):
    @pytest.fixture
    def obj(self):
        return Series([1.1, 2.2, 3.3, 4.4], dtype=float32)
...
def test_setitem_bool_int_float_consistency(indexer_sli):
    ...
    indexer_sli(ser)[0] = float64(1.0)
    assert ser.dtype == np.int64

    # 1 can be held losslessly, so no casting
    ser = Series(0, index=range(3), dtype=float64)
