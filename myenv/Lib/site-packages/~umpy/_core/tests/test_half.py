...
class TestHalf:
    ...
    def test_half_conversion_to_string(self, string_dt):
        ...
        assert np.promote_types(float16, string_dt) == expected_dt
        assert np.promote_types(string_dt, float16) == expected_dt

        arr = np.ones(3, dtype=float16).astype(string_dt)
        assert arr.dtype == expected_dt

    @pytest.mark.parametrize("string_dt", ["S", "U"])
    def test_half_conversion_from_string(self, string_dt):
        string = np.array("3.1416", dtype=string_dt)
        assert string.astype(float16) == np.array(3.1416, dtype=float16)

    @pytest.mark.parametrize("offset", [None, "up", "down"])
    @pytest.mark.parametrize("shift", [None, "up", "down"])
    @pytest.mark.parametrize("float_t", [float32, float64])
    @np._no_nep50_warning()
    def test_half_conversion_rounding(self, float_t, shift, offset):
        # Assumes that round to even is used during casting.
        max_pattern = float16(np.finfo(float16).max).view(np.uint16)

        # Test all (positive) finite numbers, denormals are most interesting
        # however:
        f16s_patterns = np.arange(0, max_pattern+1, dtype=np.uint16)
        f16s_float = f16s_patterns.view(float16).astype(float_t)
        ...
        res_patterns = f16s_float.astype(float16).view(np.uint16)
    ...
    @pytest.mark.parametrize(["float_t", "uint_t", "bits"],
                             [(float32, np.uint32, 23),
                              (float64, np.uint64, 52)])
    def test_half_conversion_denormal_round_even(self, float_t, uint_t, bits):
        ...
        # smallest denormal:
        smallest_value = np.uint16(1).view(float16).astype(float_t)
        assert smallest_value == 2**-24

        # Will be rounded to zero based on round to even rule:
        rounded_to_zero = smallest_value / float_t(2)
        assert rounded_to_zero.astype(float16) == 0
        ...
        for i in range(bits):
            ...
            assert larger_value.astype(float16) == smallest_value
