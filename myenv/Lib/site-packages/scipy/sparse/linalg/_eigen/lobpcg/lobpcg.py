...
def lobpcg(
    ...
    If ``X.dtype == float32`` and user-provided operations/multiplications
    by `A`, `B`, and `M` all preserve the ``float32`` data type,
    all the calculations and the output are in ``float32``.
    ...
    Setting the initial approximations to dtype ``float32``
    forces all iterative values to dtype ``float32`` speeding up
    ...
    in the reduced precision ``float32`` even though the initial `X`
    and thus all iterates and the output are in full ``float64``.

    >>> inv_vals = 1./vals
    >>> inv_vals = inv_vals.astype(float32)
    ...
    if not np.issubdtype(blockVectorX.dtype, np.inexact):
        warnings.warn(
            ...
            f"which is not inexact, so casted to float32.",
            UserWarning, stacklevel=2
        )
        blockVectorX = np.asarray(blockVectorX, dtype=float32)
