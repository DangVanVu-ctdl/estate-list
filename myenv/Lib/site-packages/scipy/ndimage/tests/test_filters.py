...
class TestNdimageFilters:
    ...
    def test_correlate15(self, dtype_array):
        ...
        array = np.array([[1, 2, 3],
                          [4, 5, 6]], dtype_array)
        output = ndimage.correlate(array, kernel, output=float32)
        assert_array_almost_equal([[2, 3, 5], [5, 6, 8]], output)
        assert_equal(output.dtype.type, float32)

        output = ndimage.convolve(array, kernel, output=float32)
        assert_array_almost_equal([[6, 8, 9], [9, 11, 12]], output)
        assert_equal(output.dtype.type, float32)
    ...
    def test_gauss03(self):
        ...
        input = np.arange(100 * 100).astype(float32)
        input.shape = (100, 100)
    ...
    def test_gauss04(self):
        ...
        assert_equal(output.dtype.type, float64)
        assert_equal(input.shape, output.shape)
        assert_(sumsq(input, output) > 1.0)
    ...
    def test_filter_axes(self, filter_func, extra_args, size0, size, axes):
        ...
        array = np.arange(6 * 8 * 12, dtype=float64).reshape(6, 8, 12)
        axes = np.array(axes)
    ...
    def test_minmax_nonseparable_axes(self, filter_func, axes, kwargs):
        array = np.arange(6 * 8 * 12, dtype=float32).reshape(6, 8, 12)
...
def test_valid_origins():
    ...
    def func(x):
        return np.mean(x)
    data = np.array([1, 2, 3, 4, 5], dtype=float64)
...
def test_gaussian_filter():
    # Test gaussian filter with float16
    # gh-8207
    data = np.array([1], dtype=float16)
    sigma = 1.0
    with assert_raises(RuntimeError):
