    ...
    def __init__(self, x, y, kind='linear', axis=-1,
        ...
        if kind in ('linear', 'nearest', 'nearest-up', 'previous', 'next'):
            ...
            if kind == 'nearest':
                ...
                np_dtypes = (np.dtype(float64), np.dtype(int))
...
class NdPPoly:
    ...
    def __init__(self, c, x, extrapolate=None):
        self.x = tuple(np.ascontiguousarray(v, dtype=float64) for v in x)
        self.c = np.asarray(c)
        if extrapolate is None:
            extrapolate = True
    ...
    def __call__(self, x, nu=None, extrapolate=None):
        ...
        x_shape = x.shape
        x = np.ascontiguousarray(x.reshape(-1, x.shape[-1]), dtype=float64)
