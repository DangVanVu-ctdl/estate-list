...
def riccati_jn(n, x):
    ...
    jn = np.empty((n1 + 1,), dtype = float64)
...
def riccati_yn(n, x):
    ...
    yn = np.empty((n1 + 1,), dtype = float64)
...
def lpmn(m, n, z):
    ...
    p = np.empty((m_abs + 1, n + 1) + z.shape, dtype=float64)
    pd = np.empty_like(p)
    if (z.ndim == 0):
...
def lqmn(m, n, z):
    ...
    if np.iscomplexobj(z):
        ...
        q = np.empty((mm + 1, nn + 1) + z.shape, dtype = float64)
    qd = np.empty_like(q)
    if (z.ndim == 0):
        _lqmn(z, out = (q, qd))
    else:
...
def lqn(n, z):
    ...
    if np.iscomplexobj(z):
        ...
        qn = np.empty((n1 + 1,) + z.shape, dtype=float64)
    qd = np.empty_like(qn)
    if (z.ndim == 0):
        _lqn(z, out = (qn, qd))
    else:
