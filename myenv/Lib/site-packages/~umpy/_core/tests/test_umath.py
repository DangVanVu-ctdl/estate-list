...
class TestAdd:
    def test_reduce_alignment(self):
        ...
        a = np.zeros(2, dtype=[('a', np.int32), ('b', float64)])
        a['a'] = -1
...
class TestCbrt:
    def test_cbrt_scalar(self):
        assert_almost_equal((np.cbrt(float32(-2.5)**3)), -2.5)
...
class TestPower:
    def test_power_float(self):
        ...
        for out, inp, msg in _gen_alignment_data(dtype=float32,
                                                 type='unary',
                                                 max_size=11):
        ...
        for out, inp, msg in _gen_alignment_data(dtype=float64,
                                                 type='unary',
                                                 max_size=7):
            exp = [ncu.sqrt(i) for i in inp]
            assert_almost_equal(inp**(0.5), exp, err_msg=msg)
...
class TestLog:
    ...
    def test_log_strides(self):
        ...
        for ii in sizes:
            x_f64 = float64(np.random.uniform(low=0.01, high=100.0,size=ii))
...
class TestExp:
    ...
    def test_exp_strides(self):
        ...
        sizes = np.arange(2,100)
        for ii in sizes:
            x_f64 = float64(np.random.uniform(low=0.01, high=709.1,size=ii))
            y_true = np.exp(x_f64)
            for jj in strides:
...
class TestSpecialFloats:
    ...
    def test_exp_exceptions(self):
        with np.errstate(over='raise'):
            assert_raises(FloatingPointError, np.exp, float16(11.0899))
            assert_raises(FloatingPointError, np.exp, float32(100.))
            assert_raises(FloatingPointError, np.exp, float32(1E19))
            assert_raises(FloatingPointError, np.exp, float64(800.))
            assert_raises(FloatingPointError, np.exp, float64(1E19))

        with np.errstate(under='raise'):
            assert_raises(FloatingPointError, np.exp, float16(-17.5))
            assert_raises(FloatingPointError, np.exp, float32(-1000.))
            assert_raises(FloatingPointError, np.exp, float32(-1E19))
            assert_raises(FloatingPointError, np.exp, float64(-1000.))
            assert_raises(FloatingPointError, np.exp, float64(-1E19))
    ...
    def test_avx_based_ufunc(self):
        ...
        for func, prop in avx_ufuncs.items():
            ...
            for size in range(1,32):
                ...
                x_f64 = x_f32.astype(float64)
                x_f128 = x_f32.astype(np.longdouble)
                y_true128 = myfunc(x_f128)
                if maxulperr == 0:
                    assert_equal(myfunc(x_f32), y_true128.astype(float32))
                    assert_equal(myfunc(x_f64), y_true128.astype(float64))
                else:
                    assert_array_max_ulp(myfunc(x_f32),
                                         y_true128.astype(float32),
                                         maxulp=maxulperr)
                    assert_array_max_ulp(myfunc(x_f64),
                                         y_true128.astype(float64),
                                         maxulp=maxulperr)
...
class TestAVXFloat32Transcendental:
    def test_exp_float32(self):
        np.random.seed(42)
        x_f32 = float32(np.random.uniform(low=0.0,high=88.1,size=1000000))
        x_f64 = float64(x_f32)
        assert_array_max_ulp(np.exp(x_f32), float32(np.exp(x_f64)), maxulp=3)

    def test_log_float32(self):
        np.random.seed(42)
        x_f32 = float32(np.random.uniform(low=0.0,high=1000,size=1000000))
        x_f64 = float64(x_f32)
        assert_array_max_ulp(np.log(x_f32), float32(np.log(x_f64)), maxulp=4)
    ...
    def test_sincos_float32(self):
        ...
        x_f32 = float32(np.random.uniform(low=-100.,high=100.,size=N))
        if not _glibc_older_than("2.17"):
            # test coverage for elements > 117435.992f for which glibc is used
            # this is known to be problematic on old glibc, so skip it there
            x_f32[index] = float32(10E+10*np.random.rand(M))
        x_f64 = float64(x_f32)
        assert_array_max_ulp(np.sin(x_f32), float32(np.sin(x_f64)), maxulp=2)
        assert_array_max_ulp(np.cos(x_f32), float32(np.cos(x_f64)), maxulp=2)
        # test aliasing(issue #17761)
        tx_f32 = x_f32.copy()
        assert_array_max_ulp(np.sin(x_f32, out=x_f32), float32(np.sin(x_f64)), maxulp=2)
        assert_array_max_ulp(np.cos(tx_f32, out=tx_f32), float32(np.cos(x_f64)), maxulp=2)
    ...
    def test_strided_float32(self):
        ...
        sizes = np.arange(2,100)
        for ii in sizes:
            x_f32 = float32(np.random.uniform(low=0.01,high=88.1,size=ii))
...
class TestLdexp:
    def _check_ldexp(self, tp):
        assert_almost_equal(ncu.ldexp(np.array(2., float32),
                                      np.array(3, tp)), 16.)
        assert_almost_equal(ncu.ldexp(np.array(2., float64),
                                      np.array(3, tp)), 16.)
...
class TestMaximum(_FilterInvalids):
    ...
    def test_precision(self):
        dtypes = [float16, float32, float64, np.longdouble]
...
class TestHeavside:
    def test_heaviside(self):
        ...
        h = ncu.heaviside(x, float32(0.5))
        assert_equal(h, expectedhalf.astype(float32))

        h = ncu.heaviside(x, float32(1.0))
        assert_equal(h, expected1.astype(float32))
...
class TestMinMax:
    def test_minmax_blocked(self):
        ...
        for dt, sz in [(float32, 15), (float64, 7)]:
    ...
    def test_lower_align(self):
        ...
        d = np.zeros(23 * 8, dtype=np.int8)[4:-4].view(float64)
    ...
    def test_reduce_reorder(self):
        ...
        for n in (2, 4, 8, 16, 32):
            for dt in (float32, float16, np.complex64):
...
class TestAbsoluteNegative:
    def test_abs_neg_blocked(self):
        ...
        for dt, sz in [(float32, 11), (float64, 5)]:
            for out, inp, msg in _gen_alignment_data(dtype=dt, type='unary',
                                                     max_size=sz):
...
class TestRoundingFunctions:
    ...
    @pytest.mark.parametrize('dtype', [np.bool, float64, float32,
                                       np.int64, np.uint32])
...
class TestComplexFunctions:
    ...
    def test_promotion_corner_cases(self):
        for func in self.funcs:
            assert func(float16(1)).dtype == float16
            # Integer to low precision float promotion is a dubious choice:
            assert func(np.uint8(1)).dtype == float16
            assert func(np.int16(1)).dtype == float32
...
def test_nextafter():
    return _test_nextafter(float64)


def test_nextafterf():
    return _test_nextafter(float32)
...
def test_spacing():
    return _test_spacing(float64)

def test_spacingf():
    return _test_spacing(float32)
...
def test_spacing_gfortran():
    ...
    #       END PROGRAM
    ref = {float64: [1.69406589450860068E-021,
                        2.22044604925031308E-016,
                        1.13686837721616030E-013,
                        1.81898940354585648E-012],
           float32: [9.09494702E-13,
                        1.19209290E-07,
                        6.10351563E-05,
                        9.76562500E-04]}

    for dt, dec_ in zip([float32, float64], (10, 20)):
...
def test_reduceat():
    ...
    db = np.dtype([('name', 'S11'), ('time', np.int64), ('value', float32)])
...
@pytest.mark.parametrize('ftype', [float32, float64])
...
def test_bad_legacy_ufunc_silent_errors():
    ...
    arr = np.arange(3).astype(float64)
