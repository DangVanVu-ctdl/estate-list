...
class TestMultiIndexLoc:
    ...
    def test_loc_getitem_series(self):
        ...
        x = Series(index=index, data=range(9), dtype=float64)
        y = Series([1, 3])
        expected = Series(
            data=[0, 1, 2, 6, 7, 8],
        ...
        empty = Series(data=[], dtype=float64)
        expected = Series(
            [],
            index=MultiIndex(levels=index.levels, codes=[[], []], dtype=float64),
            dtype=float64,
        )
        result = x.loc[empty]
        tm.assert_series_equal(result, expected)
    ...
    def test_loc_getitem_array(self):
        ...
        # 0-dim array (scalar):
        scalar = np.int64(1)
        expected = Series(data=[0, 1, 2], index=["A", "B", "C"], dtype=float64)
        result = x.loc[scalar]
        tm.assert_series_equal(result, expected)
...
def test_3levels_leading_period_index():
    ...
    ser = Series(range(4), index=mi, dtype=float64)
...
def test_loc_empty_single_selector_with_names():
    ...
    s2 = Series(index=idx, dtype=float64)
    result = s2.loc["a"]
