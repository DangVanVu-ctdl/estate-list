...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Support below float32 is rare and far between.
        ({"downcast": "float"}, np.dtype(float32).char),
        # Basic dtype support.
...
@pytest.mark.parametrize(
    "data,expected",
    [
        (["1.1", 2, 3], np.array([1.1, 2, 3], dtype=float64)),
        (
            [10000.0, 20000, 3000, 40000.36, 50000, 50000.00],
            np.array(
                [10000.0, 20000, 3000, 40000.36, 50000, 50000.00], dtype=float64
            ),
        ),
    ],
)
...
def test_downcast_float64_to_float32():
    ...
    series = Series([16777217.0, np.finfo(float64).max, np.nan], dtype=float64)
