...
@pytest.mark.parametrize('tp', [float32, np.double, np.longdouble])
def test_float_types(tp):
    """ Check formatting.

        This is only for the str function, and only for simple types.
        The precision of float32 and np.longdouble aren't the same as the
...
class TestCommaDecimalPointLocale(CommaDecimalPointLocale):
    ...
    def test_locale_single(self):
        assert_equal(str(float32(1.2)), str(float(1.2)))
