...
class _TestCommon:
    ...
    def init_class(cls):
        ...
        assert_equal(cls.dat, cls.dat_dtypes[float64])
        assert_equal(cls.datsp.toarray(),
                     cls.datsp_dtypes[float64].toarray())
    ...
    def test_rmul_scalar_type_error(self):
        datsp = self.datsp_dtypes[float64]
    ...
    def test_maximum_minimum(self):
        ...
        for dtype in self.math_dtypes:
            for dtype2 in [np.int8, float64, np.complex128]:
    ...
    def test_constructor1_base(self):
        ...
        C = A.__class__(A, dtype=float32, copy=False)
...
class _TestSlicing:
    def test_dtype_preservation(self):
        ...
        assert_equal(self.spcreator((1,10), dtype=float32)[0,1:5].dtype, float32)
        assert_equal(self.spcreator((1,10), dtype=float64)[0,1:5].dtype, float64)
    ...
    def test_dtype_preservation_empty_slice(self):
        ...
        for dt in [np.int16, np.int32, float32, float64]:
...
class _TestMinMax:
    def test_minmax(self):
        for dtype in [float32, float64, np.int32, np.int64, np.complex128]:
...
class TestCSR(sparse_test_class()):
    ...
    math_dtypes = [np.bool_, np.int_, float64, np.complex128]
...
class TestDOK(sparse_test_class(minmax=False, nnz_axis=False)):
    ...
    math_dtypes = [np.int_, float64, np.complex128]
    ...
    def test_ctor(self):
        ...
        data = [[0, 1, 2], [3, 0, 0]]
        d = dok_matrix(data, dtype=float32)
        assert_equal(d.dtype, float32)
        da = d.toarray()
        assert_equal(da.dtype, float32)
...
class _NonCanonicalMixin:
    def spcreator(self, D, sorted_indices=False, **kwargs):
        ...
        if NC.dtype in [float32, np.complex64]:
