...
class TestTieCorrect:
    ...
    def test_empty(self):
        ...
        ranks = np.array([], dtype=float64)
        c = tiecorrect(ranks)
        assert_equal(c, 1.0)

    def test_one(self):
        """A single element requires no correction, should return 1.0."""
        ranks = np.array([1.0], dtype=float64)
        c = tiecorrect(ranks)
        assert_equal(c, 1.0)
...
class TestRankData:
    ...
    def test_empty(self):
        ...
        r = rankdata(a)
        assert_array_equal(r, np.array([], dtype=float64))
        r = rankdata([])
        assert_array_equal(r, np.array([], dtype=float64))
    ...
    def test_one(self):
        ...
        r = rankdata(a)
        assert_array_equal(r, np.array([1.0], dtype=float64))
        r = rankdata(data)
        assert_array_equal(r, np.array([1.0], dtype=float64))
    ...
    def test_basic(self):
        ...
        data = [100, 10, 50]
        expected = np.array([3.0, 1.0, 2.0], dtype=float64)
        a = np.array(data, dtype=int)
        r = rankdata(a)
        ...
        data = [40, 10, 30, 10, 50]
        expected = np.array([4.0, 1.5, 3.0, 1.5, 5.0], dtype=float64)
        a = np.array(data, dtype=int)
        r = rankdata(a)
        assert_array_equal(r, expected)
        r = rankdata(data)
        ...
        expected = np.array([5.0, 5.0, 5.0, 2.0, 2.0, 2.0], dtype=float64)
        a = np.array(data, dtype=int)
        r = rankdata(a)
        assert_array_equal(r, expected)
        r = rankdata(data)
