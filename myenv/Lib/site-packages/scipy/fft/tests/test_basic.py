...
class TestFFT:
    ...
    @pytest.mark.parametrize("dtype", [float16, np.longdouble])
    def test_dtypes_nonstandard(self, dtype):
        x = random(30).astype(dtype)
        out_dtypes = {float16: np.complex64, np.longdouble: np.clongdouble}
        ...
        assert res_rfft.dtype == np.result_type(float32, x.dtype)
        assert res_hfft.dtype == np.result_type(float32, x.dtype)
