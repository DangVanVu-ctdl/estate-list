...
class genhyperbolic_gen(rv_continuous):
    ...
    @lambda func: np.vectorize(func.__get__(object), otypes=[float64])
    @staticmethod
    ...
    def _rvs(self, p, a, b, size=None, random_state=None):
        ...
        t1 = float_power(a, 2) - float_power(b, 2)
        # b in the GIG
        t2 = float_power(t1, 0.5)
        # scale in the GIG
        t3 = float_power(t1, - 0.5)
        gig = geninvgauss.rvs(
            p=p,
            b=t2,
            scale=t3,
    ...
    def _stats(self, p, a, b):
        ...
        p, a, b = np.broadcast_arrays(p, a, b)
        t1 = float_power(a, 2) - float_power(b, 2)
        t1 = float_power(t1, 0.5)
        t2 = float_power(1, 2) * float_power(t1, - 1)
        ...
        m = b * t2 * r1
        v = (
            t2 * r1 + float_power(b, 2) * float_power(t2, 2) *
            (r2 - float_power(r1, 2))
        )
        m3e = (
            float_power(b, 3) * float_power(t2, 3) *
            (r3 - 3 * b2 * b1 * float_power(b0, -2) +
             2 * float_power(r1, 3)) +
            3 * b * float_power(t2, 2) *
            (r2 - float_power(r1, 2))
        )
        s = m3e * float_power(v, - 3 / 2)
        m4e = (
            float_power(b, 4) * float_power(t2, 4) *
            (r4 - 4 * b3 * b1 * float_power(b0, - 2) +
             6 * b2 * float_power(b1, 2) * float_power(b0, - 3) -
             3 * float_power(r1, 4)) +
            float_power(b, 2) * float_power(t2, 3) *
            (6 * r3 - 12 * b2 * b1 * float_power(b0, - 2) +
             6 * float_power(r1, 3)) +
            3 * float_power(t2, 2) * r2
        )
        k = m4e * float_power(v, -2) - 3
...
class geninvgauss_gen(rv_continuous):
    ...
    def _logpdf(self, x, p, b):
        ...
        logpdf_single = np.vectorize(logpdf_single, otypes=[float64])
    ...
    def _cdf(self, x, *args):
        ...
        _cdf_single = np.vectorize(_cdf_single, otypes=[float64])
    ...
    def _munp(self, n, p, b):
        ...
        if inf_vals.any():
            ...
            warnings.warn(msg, RuntimeWarning, stacklevel=3)
            m = np.full_like(num, np.nan, dtype=float64)
...
class jf_skew_t_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        ...
        return _lazywhere(
            nth_moment_valid,
            (n, a, b),
            np.vectorize(nth_moment, otypes=[float64]),
            np.nan,
        )
...
class irwinhall_gen(rv_continuous):
    ...
    def _munp(self, order, n):
        ...
        return np.vectorize(vmunp, otypes=[float64])(order, n)
    ...
    def _pdf(self, x, n):
        ...
        return np.vectorize(vpdf, otypes=[float64])(x, n)
    
    def _cdf(self, x, n):
        def vcdf(x, n):
            return self._cardbspl(n).antiderivative()(x)
        return np.vectorize(vcdf, otypes=[float64])(x, n)

    def _sf(self, x, n):
        def vsf(x, n):
            return self._cardbspl(n).antiderivative()(n-x)
        return np.vectorize(vsf, otypes=[float64])(x, n)
...
class truncnorm_gen(rv_continuous):
    ...
    def _munp(self, n, a, b):
        ...
        return _lazywhere((n >= 0) & (a == a) & (b == b), (n, a, b),
                          np.vectorize(n_th_moment, otypes=[float64]),
                          np.nan)
...
class studentized_range_gen(rv_continuous):
    ...
    def _munp(self, K, k, df):
        ...
        return np.asarray(ufunc(K, k, df), dtype=float64)[()]
    ...
    def _pdf(self, x, k, df):
        ...
        return np.asarray(ufunc(x, k, df), dtype=float64)[()]
    ...
    def _cdf(self, x, k, df):
        ...
        return np.clip(np.asarray(ufunc(x, k, df), dtype=float64)[()], 0, 1)
