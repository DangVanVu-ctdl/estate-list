...
def _calculate_deltas(
    ...
    if isinstance(times, ABCSeries):
        times = times._values
    _times = np.asarray(times.view(np.int64), dtype=float64)
        ...
        *,
        ...
        if self.times is not None:
            ...
            self._deltas = np.ones(
                max(self.obj.shape[self.axis] - 1, 0), dtype=float64
            )
...
class OnlineExponentialMovingWindow(ExponentialMovingWindow):
    ...
    def mean(self, *args, update=None, update_times=None, **kwargs):
        ...
        update_deltas = np.ones(
            max(self._selected_obj.shape[self.axis - 1] - 1, 0), dtype=float64
        )
        if update is not None:
            ...
            np_array = self._selected_obj.astype(float64, copy=False).to_numpy()
