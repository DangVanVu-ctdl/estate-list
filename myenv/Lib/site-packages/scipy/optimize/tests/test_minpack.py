...
class TestFSolve:
    ...
    def test_float32(self):
        def func(x):
            return np.array([x[0] - 100, x[1] - 1000], dtype=float32) ** 2
        p = optimize.fsolve(func, np.array([1, 1], float32))
...
class TestCurveFit:
    ...
    def test_dtypes(self):
        ...
        for method in ['lm', 'trf', 'dogbox']:
            for dtx in [float32, float64]:
                for dty in [float32, float64]:
                    x = x.astype(dtx)
                    y = y.astype(dty)
