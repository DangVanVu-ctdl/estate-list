...
class TestNonarrayArgs:
    ...
    def test_round(self):
        ...
        s = float64(1.)
        assert_(isinstance(s.round(), float64))
    ...
    def test_dunder_round_accuracy(self):
        f = float64(5.1 * 10**73)
        assert_(isinstance(round(f, -73), float64))
        assert_array_max_ulp(round(f, -73), 5.0 * 10**73)
        assert_(isinstance(round(f, ndigits=-73), float64))
    ...
    def test_round_py_consistency(self):
        f = 5.1 * 10**73
        assert_equal(round(float64(f), -73), round(f, -73))
...
class TestBoolCmp:
    def setup_method(self):
        self.f = np.ones(256, dtype=float32)
        self.ef = np.ones(self.f.size, dtype=bool)
        self.d = np.ones(128, dtype=float64)
...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        f32, f64, fld = float32(0), float64(0), np.longdouble(0)
        ...
        assert_equal(promote_func(u64, np.array([i32])), np.dtype(float64))
        ...
        assert_equal(promote_func(np.int32(-1), np.array([u64])),
                     np.dtype(float64))
        assert_equal(promote_func(f64, np.array([f32])), np.dtype(float64))
        ...
        assert_equal(promote_func(np.array([b]), f64), np.dtype(float64))
        assert_equal(promote_func(np.array([b]), i64), np.dtype(np.int64))
        assert_equal(promote_func(np.array([b]), u64), np.dtype(np.uint64))
        assert_equal(promote_func(np.array([i8]), f64), np.dtype(float64))
        assert_equal(promote_func(np.array([u16]), f64), np.dtype(float64))
    ...
    def test_coercion(self):
        ...
        #
        # a = np.array([1.0, 1.5], dtype=float32)
    ...
    def test_can_cast(self):
        ...
        assert_(np.can_cast(float64, complex))
...
class TestClip:
    ...
    @given(
        data=st.data(),
        arr=hynp.arrays(
            dtype=hynp.integer_dtypes() | hyfloating_dtypes(),
    ...
    def test_clip_property(self, data, arr):
        ...
        """
        numeric_dtypes = hynp.integer_dtypes() | hyfloating_dtypes()
...
class TestIsclose:
    ...
    def test_nep50_isclose(self):
        ...
        assert np.isclose(f32, float32(0), atol=below_one)
        assert np.isclose(f32, 2, atol=0, rtol=below_one/2)
        assert not np.isclose(f32, float64(below_one), atol=0, rtol=0)
        assert not np.isclose(f32, float32(0), atol=float64(below_one))
        assert not np.isclose(f32, 2, atol=0, rtol=float64(below_one/2))
...
class TestIndices:
    ...
    @pytest.mark.parametrize("dtype", [np.int32, np.int64, float32, float64])
