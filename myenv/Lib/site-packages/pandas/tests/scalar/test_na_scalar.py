...
@pytest.mark.parametrize(
    "value", [1, 1.0, True, np.bool_(True), np.int_(1), float64(1)]
)
@pytest.mark.parametrize("asarray", [True, False])
def test_rpow_special(value, asarray):
    ...
    if asarray:
        result = result[0]
    elif not isinstance(value, (float64, np.bool_, np.int_)):
...
@pytest.mark.parametrize("value", [-1, -1.0, np.int_(-1), float64(-1)])
@pytest.mark.parametrize("asarray", [True, False])
