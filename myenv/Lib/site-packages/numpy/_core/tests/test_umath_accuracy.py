...
def convert(s, datatype="float32"):
...
class TestAccuracy:
    ...
    def test_validate_fp16_transcendentals(self, ufunc):
        with np.errstate(all='ignore'):
            ...
            datafp16 = np.frombuffer(arr.tobytes(), dtype=float16)
            datafp32 = datafp16.astype(float32)
            assert_array_max_ulp(ufunc(datafp16), ufunc(datafp32),
                    maxulp=1, dtype=float16)
