...
def _constraints_to_components(constraints):
    ...
    for constraint in constraints:
        ...
        b_ls.append(np.atleast_1d(constraint.lb).astype(float64))
        b_us.append(np.atleast_1d(constraint.ub).astype(float64))
...
def _milp_iv(c, integrality, bounds, constraints, options):
    ...
    c = np.atleast_1d(c).astype(float64)
    ...
    try:
        lb = np.broadcast_to(bounds.lb, c.shape).astype(float64)
        ub = np.broadcast_to(bounds.ub, c.shape).astype(float64)
    ...
    if A.shape != (b_l.size, c.size):
        ...
        raise ValueError(message)
    indptr, indices, data = A.indptr, A.indices, A.data.astype(float64)
