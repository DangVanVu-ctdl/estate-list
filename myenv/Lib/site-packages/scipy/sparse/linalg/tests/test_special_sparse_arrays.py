...
class TestLaplacianNd:
    ...
    def test_eigenvalues(self, grid_shape, bc):
        lap = LaplacianNd(grid_shape, boundary_conditions=bc, dtype=float64)
        L = lap.toarray()
...
class TestMikotaPair:
    ...
    # so its smallest still accurate `dtype` is `float32`
    ...
    def test_linearoperator_shape_dtype(self, dtype):
        ...
        assert mikd_k.dtype == float64
        assert mikd_m.shape == (n, n)
        assert mikd_m.dtype == float64
