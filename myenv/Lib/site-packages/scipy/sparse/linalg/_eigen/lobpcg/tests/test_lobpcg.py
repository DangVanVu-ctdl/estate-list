...
def test_b_orthonormalize(n, m, Vdtype, Bdtype, BVdtype):
    ...
    atol = m * n * max(np.finfo(dtype).eps, np.finfo(float64).eps)
...
def test_nonhermitian_warning(capsys):
    ...
    """
    n = 10
    X = np.arange(n * 2).reshape(n, 2).astype(float32)
    A = np.arange(n * n).reshape(n, n).astype(float32)
...
def test_diagonal_data_types(n, m):
    ...
    for s_f_i, s_f in enumerate(list_sparse_format):
        ...
        As32 = As64.astype(float32)
        Af64 = As64.toarray()
        Af32 = Af64.astype(float32)
        ...
        Yf32 = np.eye(n, m_excluded, dtype=float32)
        listY = [Yf64, Yf32]
