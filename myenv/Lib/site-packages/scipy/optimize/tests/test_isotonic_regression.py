...
class TestIsotonicRegression:
    ...
    def test_simple_pava(self):
        ...
        y = np.array([8, 4, 8, 2, 2, 0, 8], dtype=float64)
        w = np.ones_like(y)
    ...
    @pytest.mark.parametrize("y_dtype", [float64, float32, np.int64, np.int32])
    @pytest.mark.parametrize("w_dtype", [float64, float32, np.int64, np.int32])
    ...
    def test_simple_isotonic_regression(self, w, w_dtype, y_dtype):
        ...
        assert res.x.dtype == float64
        assert res.weights.dtype == float64
        ...
        assert_equal(y, np.array([8, 4, 8, 2, 2, 0, 8], dtype=float64))
