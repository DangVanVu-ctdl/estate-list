...
class TestDataFrameSetitemCoercion:
    ...
    def test_loc_setitem_multiindex_columns(self, consolidate):
        ...
        A = DataFrame(np.zeros((6, 5), dtype=float32))
        A = pd.concat([A, A], axis=1, keys=[1, 2])
        if consolidate:
            A = A._consolidate()

        A.loc[2:3, (1, slice(2, 3))] = np.ones((2, 2), dtype=float32)
        assert (A.dtypes == float32).all()

        A.loc[0:5, (1, slice(2, 3))] = np.ones((6, 2), dtype=float32)

        assert (A.dtypes == float32).all()

        A.loc[:, (1, slice(2, 3))] = np.ones((6, 2), dtype=float32)
        assert (A.dtypes == float32).all()
...
def test_26395(indexer_al):
    ...
    expected = DataFrame({"D": [0, 0, 44.5]}, index=["A", "B", "C"], dtype=float64)
    tm.assert_frame_equal(df, expected)
...
def test_15231():
    ...
    exp_dtypes = Series([np.int64, float64], dtype=object, index=["a", "b"])
...
def test_iloc_setitem_unnecesssary_float_upcasting():
    ...
    df = DataFrame(
        {
            0: np.array([1, 3], dtype=float32),
            1: np.array([2, 4], dtype=float32),
            2: ["a", "b"],
        }
    )
    orig = df.copy()
...
def test_20476():
    ...
    expected.columns = mi
    exp_dtypes = Series(
        [np.dtype(np.int64)] * 2 + [np.dtype(float64)] + [np.dtype(np.int64)] * 3,
        index=mi,
    )
    tm.assert_series_equal(df.dtypes, exp_dtypes)
