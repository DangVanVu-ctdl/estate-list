...
class TestLU:
    ...
    def test_1by1_input_output(self):
        ...
        assert_allclose(l, np.ones(shape=(4, 5, 1, 1), dtype=float32))
        assert_allclose(u, a)

        a = self.rng.random([4, 5, 1, 1], dtype=float32)
        p, l, u = lu(a)
        assert_allclose(p, np.ones(shape=(4, 5, 1, 1), dtype=float32))
        assert_allclose(l, np.ones(shape=(4, 5, 1, 1), dtype=float32))
        assert_allclose(u, a)

        pl, u = lu(a, permute_l=True)
        assert_allclose(pl, np.ones(shape=(4, 5, 1, 1), dtype=float32))
        assert_allclose(u, a)
    ...
    def test_empty_edge_cases(self):
        a = np.empty([0, 0])
        p, l, u = lu(a)
        assert_allclose(p, np.empty(shape=(0, 0), dtype=float64))
        assert_allclose(l, np.empty(shape=(0, 0), dtype=float64))
        assert_allclose(u, np.empty(shape=(0, 0), dtype=float64))

        a = np.empty([0, 3], dtype=float16)
        p, l, u = lu(a)
        assert_allclose(p, np.empty(shape=(0, 0), dtype=float32))
        assert_allclose(l, np.empty(shape=(0, 0), dtype=float32))
        assert_allclose(u, np.empty(shape=(0, 3), dtype=float32))
        ...
        p, l, u = lu(a)
        assert_allclose(p, np.empty(shape=(3, 0, 0), dtype=float32))
