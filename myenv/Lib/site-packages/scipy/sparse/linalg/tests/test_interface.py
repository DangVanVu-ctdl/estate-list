...
class TestAsLinearOperator:
    def setup_method(self):
        ...
        self.cases += make_cases(original, float32)
        self.cases += make_cases(original, float64)
        self.cases += [(interface.aslinearoperator(M).T, A.T)
                       for M, A in make_cases(original.T, float64)]
...
def test_dtypes_of_operator_sum():
    ...
    assert_equal(sum_real.dtype, float64)
