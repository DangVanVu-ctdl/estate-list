...
def test_array_array():
    tobj = type(object)
    ones11 = np.ones((1, 1), float64)
    tndarray = type(ones11)
    # Test is_ndarray
    assert_equal(np.array(ones11, dtype=float64), ones11)
    if HAS_REFCOUNT:
    ...
    # test None
    assert_equal(np.array(None, dtype=float64),
                 np.array(np.nan, dtype=float64))
    if HAS_REFCOUNT:
        old_refcount = sys.getrefcount(tobj)
        np.array(None, dtype=float64)
        assert_equal(old_refcount, sys.getrefcount(tobj))

    # test scalar
    assert_equal(np.array(1.0, dtype=float64),
                 np.ones((), dtype=float64))
    if HAS_REFCOUNT:
        old_refcount = sys.getrefcount(float64)
        np.array(np.array(1.0, dtype=float64), dtype=float64)
        assert_equal(old_refcount, sys.getrefcount(float64))

    # test string
    S2 = np.dtype((bytes, 2))
    S3 = np.dtype((bytes, 3))
    S5 = np.dtype((bytes, 5))
    assert_equal(np.array(b"1.0", dtype=float64),
                 np.ones((), dtype=float64))
    ...
    # test memoryview
    dat = np.array(memoryview(b'1.0'), dtype=float64)
    assert_equal(dat, [49.0, 46.0, 48.0])
    assert_(dat.dtype.type is float64)
    ...
    a = np.array(100.0, dtype=float64)
    o = type("o", (object,),
             dict(__array_interface__=a.__array_interface__))
    assert_equal(np.array(o, dtype=float64), a)
    ...
    assert_equal(np.array(o, dtype=float64), np.array(100.0, float64))
    ...
    assert_raises(ValueError, np.array, [nested], dtype=float64)

    # Try with lists...
    # float32
    assert_equal(np.array([None] * 10, dtype=float32),
                 np.full((10,), np.nan, dtype=float32))
    assert_equal(np.array([[None]] * 10, dtype=float32),
                 np.full((10, 1), np.nan, dtype=float32))
    assert_equal(np.array([[None] * 10], dtype=float32),
                 np.full((1, 10), np.nan, dtype=float32))
    assert_equal(np.array([[None] * 10] * 10, dtype=float32),
                 np.full((10, 10), np.nan, dtype=float32))
    # float64
    assert_equal(np.array([None] * 10, dtype=float64),
                 np.full((10,), np.nan, dtype=float64))
    assert_equal(np.array([[None]] * 10, dtype=float64),
                 np.full((10, 1), np.nan, dtype=float64))
    assert_equal(np.array([[None] * 10], dtype=float64),
                 np.full((1, 10), np.nan, dtype=float64))
    assert_equal(np.array([[None] * 10] * 10, dtype=float64),
                 np.full((10, 10), np.nan, dtype=float64))

    assert_equal(np.array([1.0] * 10, dtype=float64),
                 np.ones((10,), dtype=float64))
    assert_equal(np.array([[1.0]] * 10, dtype=float64),
                 np.ones((10, 1), dtype=float64))
    assert_equal(np.array([[1.0] * 10], dtype=float64),
                 np.ones((1, 10), dtype=float64))
    assert_equal(np.array([[1.0] * 10] * 10, dtype=float64),
...
def test_copyto_cast_safety():
    ...
    with pytest.warns(RuntimeWarning):
        np.copyto(np.arange(3, dtype=float32), 2e300, casting="safe")
...
def test_astype_copyflag():
    ...
    res_false_float = arr.astype(float64, copy=False)
    assert not np.shares_memory(arr, res_false_float)

    # _CopyMode enum isn't allowed
    assert_raises(ValueError, arr.astype, float64,
