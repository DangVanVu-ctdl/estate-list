...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((float16, np.int16), float32),
        ((float32, np.int16), float32),
        ((np.uint64, np.int64), float64),
        ((np.int16, float64), float64),
        ((float16, np.int64), float64),
        # Into others.
        ...
        # Bool with float.
        ((np.dtype("bool"), float64), object),
        ((np.dtype("bool"), float32), object),
        (
...
def test_interval_dtype(left, right):
    ...
    if left is right:
        ...
        if right.subtype.kind in ["i", "u", "f"]:
            ...
            expected = IntervalDtype(float64, "right")
