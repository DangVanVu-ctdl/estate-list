...
def _get_test_tolerance(type_char, mattype=None, D_type=None, which=None):
    ...
    rtol = {'f': 3000 * np.finfo(float32).eps,
            'F': 3000 * np.finfo(float32).eps,
            'd': 2000 * np.finfo(float64).eps,
            'D': 2000 * np.finfo(float64).eps}[type_char]
    atol = rtol
    tol = 0
