...
class HiddenMarkovModelTagger(TaggerI):
    ...
    def _create_cache(self):
        ...
        if not self._cache:
            N = len(self._states)
            M = len(self._symbols)
            P = np.zeros(N, float32)
            X = np.zeros((N, N), float32)
            O = np.zeros((N, M), float32)
            for i in range(N):
                si = self._states[i]
    ...
    def _update_cache(self, symbols):
        ...
        if symbols:
            ...
            if not self._cache:
                ...
                O = np.hstack([O, np.zeros((N, M - Q), float32)])
                for i in range(N):
                    si = self._states[i]
    ...
    def point_entropy(self, unlabeled_sequence):
        ...
        entropies = np.zeros(T, float64)
        probs = np.zeros(N, float64)
        for t in range(T):
            for s in range(N):
    ...
    def _transitions_matrix(self):
        ...
        transitions_logprob = np.fromiter(trans_iter, dtype=float64)
        N = len(self._states)
    ...
    def _outputs_vector(self, symbol):
        ...
        return np.fromiter(out_iter, dtype=float64)
...
def _market_hmm_example():
    ...
    A = np.array([[0.6, 0.2, 0.2], [0.5, 0.3, 0.2], [0.4, 0.1, 0.5]], float64)
    B = np.array([[0.7, 0.1, 0.2], [0.1, 0.6, 0.3], [0.3, 0.3, 0.4]], float64)
    pi = np.array([0.5, 0.2, 0.3], float64)
