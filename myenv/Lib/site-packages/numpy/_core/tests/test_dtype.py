...
class TestRecord:
    ...
    def test_from_dict_with_zero_width_field(self):
        ...
        dt = np.dtype([('val1', float32, (0,)), ('val2', int)])
        dt2 = np.dtype({'names': ['val1', 'val2'],
                        'formats': [(float32, (0,)), int]})
    ...
    def test_fields_by_index(self):
        dt = np.dtype([('a', np.int8), ('b', float32, 3)])
        assert_dtype_equal(dt[0], np.dtype(np.int8))
        assert_dtype_equal(dt[1], np.dtype((float32, 3)))
...
class TestStructuredDtypeSparseFields:
    ...
    def test_sparse_field_assignment(self):
        ...
        sparse_arr[...] = np.finfo(float32).max
    ...
    def test_sparse_field_assignment_fancy(self):
        ...
        sparse_arr[[0, 1, 2]] = np.finfo(float32).max
        # dtype is reduced when accessing the field, so shape is (3, 2, 3):
        assert_array_equal(arr["a"]["aa"], np.zeros((3, 2, 3)))
...
class TestPromotion:
    ...
    @pytest.mark.parametrize(["other", "expected", "expected_weak"],
            [(2**16-1, np.complex64, None),
             ...
             (float16(2), np.complex64, None),
             (float32(2), np.complex64, None),
    ...
    @pytest.mark.parametrize(["dtypes", "expected"], [
             ...
             ([np.uint16, np.int16, float16], float32),
             ([np.uint16, np.int8, float16], float32),
             ([np.uint8, np.int16, float16], float32),
             ...
             ([1, 1j, float64], np.complex128),
             ([1., 1., np.int64], float64),
             ([1., 1j, float64], np.complex128),
             ([1j, 1j, float64], np.complex128),
...
def test_keyword_argument():
    ...
    assert np.dtype(dtype=float64) == np.dtype(float64)
