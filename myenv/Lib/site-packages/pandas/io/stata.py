...
def _datetime_to_stata_elapsed_vec(dates: Series, fmt: str) -> Series:
    ...
    conv_dates = Series(conv_dates, dtype=float64, copy=False)
...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    for col in data:
        ...
        if dtype == np.int8 and not empty_df:
            ...
            value = data[col].max()
            if dtype == float32 and value > float32_max:
                data[col] = data[col].astype(float64)
            elif dtype == float64:
...
class StataMissingValue:
    ...
    float32 missing values (float in Stata)
    0000007f    .
    0008007f    .a
    0010007f    .b
    ...
    ...
    float64 missing values (double in Stata)
    000000000000e07f    .
    000000000001e07f    .a
    000000000002e07f    .b
    ...
    ...
    def get_base_missing_value(cls, dtype: np.dtype) -> float:
        ...
        elif dtype.type is float32:
            value = cls.BASE_MISSING_VALUES["float32"]
        elif dtype.type is float64:
    ...
    def __init__(self) -> None:
        ...
        self.DTYPE_MAP = dict(
            ...
            + [
                ...
                (254, np.dtype(float32)),
                (255, np.dtype(float64)),
            ]
        )
        self.DTYPE_MAP_XML: dict[int, np.dtype] = {
            32768: np.dtype(np.uint8),  # Keys to GSO
            65526: np.dtype(float64),
            65527: np.dtype(float32),
        ...
        self.VALID_RANGE = {
            ...
            "f": (
                float32(struct.unpack("<f", float32_min)[0]),
                float32(struct.unpack("<f", float32_max)[0]),
            ),
            "d": (
                float64(struct.unpack("<d", float64_min)[0]),
        ...
        self.MISSING_VALUES = {
            ...
            "h": 32741,
            "l": 2147483621,
            "f": float32(struct.unpack("<f", b"\x00\x00\x00\x7f")[0]),
            "d": float64(
...
the string values returned are correct."""
    ...
    def read(
        ...
        if not preserve_dtypes:
            ...
            for col in data:
                dtype = data[col].dtype
                if dtype in (np.dtype(float16), np.dtype(float32)):
...
def _convert_datetime_to_stata_type(fmt: str) -> np.dtype:
    ...
    if fmt in [
        ...
        return np.dtype(float64)  # Stata expects doubles for SIFs
    else:
