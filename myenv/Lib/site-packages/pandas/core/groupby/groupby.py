...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def _wrap_aggregated_output(
        ...
        qs: npt.NDArray[float64] | None = None,
    ):
        """
    ...
    def _cumcount_array(self, ascending: bool = True) -> np.ndarray:
        ...
        if self._grouper.has_dropped_na:
            out = np.where(ids == -1, np.nan, out.astype(float64, copy=False))
        else:
    ...
    def quantile(
        ...
        def blk_func(values: ArrayLike) -> ArrayLike:
            ...
            out = np.empty((ncols, ngroups, nqs), dtype=float64)
...
def _insert_quantile_level(idx: Index, qs: npt.NDArray[float64]) -> MultiIndex:
    """
