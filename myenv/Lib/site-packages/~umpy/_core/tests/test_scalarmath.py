...
floating_types = floating.__subclasses__()
...
@pytest.mark.parametrize("fscalar", [float16, float32])
...
class TestBaseMath:
    ...
    def test_blocked(self):
        ...
        for dt, sz in [(float32, 11), (float64, 7), (np.int32, 11)]:
    ...
    def test_lower_align(self):
        ...
        o = np.zeros(23 * 8, dtype=np.int8)[4:-4].view(float64)
...
class TestPower:
    def test_small_types(self):
        for t in [np.int8, np.int16, float16]:
            a = t(3)
            b = a ** 4
            assert_(b == 81, "error with %r: got %r" % (t, b))

    def test_large_types(self):
        for t in [np.int32, np.int64, float32, float64, np.longdouble]:
            a = t(51)
            b = a ** 4
    ...
    def test_integers_to_negative_integer_power(self):
        ...
        # has common type float64. The other combinations should all
        ...
        for i1, i2 in itertools.product(base, exp):
            if i1.dtype != np.uint64:
                ...
                assert_(res.dtype.type is float64)
    ...
    def test_mixed_types(self):
        typelist = [np.int8, np.int16, float16,
                    float32, float64, np.int8,
    ...
    def test_modular_power(self):
        ...
        for t in (np.int32, float32, np.complex64):
...
class TestConversion:
    def test_int_from_long(self):
        ...
        for T in [None, float64, np.int64]:
    ...
    def test_scalar_comparison_to_none(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert_(not float32(1) == None)
            assert_(not np.str_('test') == None)
            # This is dubious (see below):
            assert_(not np.datetime64('NaT') == None)

            assert_(float32(1) != None)
...
class TestMultiply:
    ...
    def test_no_seq_repeat_basic_array_like(self):
        ...
        for arr_like in (ArrayLike(np.ones(3)), memoryview(np.ones(3))):
            assert_array_equal(arr_like * float32(3.), np.full(3, 3.))
            assert_array_equal(float32(3.) * arr_like, np.full(3, 3.))
...
@pytest.mark.parametrize("sctype", [float32, float64, np.longdouble])
...
@pytest.mark.parametrize(["__op__", "__rop__", "op", "cmp"], ops_with_names)
@pytest.mark.parametrize("subtype", [float, int, complex, float16])
...
def test_pyscalar_subclasses(subtype, __op__, __rop__, op, cmp):
    ...
    assert op(myt(1), float64(2)) == __op__
    assert op(float64(1), myt(2)) == __rop__
    ...
    res = op(myt(1), float16(2))
    expected = op(behaves_like(1), float16(2))
    assert res == expected
    assert type(res) == type(expected)
    res = op(float32(2), myt(1))
    expected = op(float32(2), behaves_like(1))
    assert res == expected
    assert type(res) == type(expected)
...
def test_truediv_int():
    ...
    assert np.uint8(3) / 123454 == float64(3) / 123454
