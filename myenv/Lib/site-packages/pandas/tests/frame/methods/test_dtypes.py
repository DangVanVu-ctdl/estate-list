...
class TestDataFrameDataTypes:
    def test_empty_frame_dtypes(self):
        ...
        ex_dtypes = Series({"a": np.int64, "b": np.bool_, "c": float64})
    ...
    def test_dtypes_are_correct_after_column_slice(self):
        # GH6525
        df = DataFrame(index=range(5), columns=list("abc"), dtype=float64)
        tm.assert_series_equal(
            df.dtypes,
            Series({"a": float64, "b": float64, "c": float64}),
        )
        tm.assert_series_equal(df.iloc[:, 2:].dtypes, Series({"c": float64}))
        tm.assert_series_equal(
            df.dtypes,
            Series({"a": float64, "b": float64, "c": float64}),
        )
