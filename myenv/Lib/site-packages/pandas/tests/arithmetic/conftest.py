...
zeros = [
    ...
    for dtype in [np.int64, np.uint64, float64]
]
zeros.extend([box_cls([-0.0] * 5, dtype=float64) for box_cls in [Index, np.array]])
zeros.extend([np.array(0, dtype=dtype) for dtype in [np.int64, np.uint64, float64]])
zeros.extend([np.array(-0.0, dtype=float64)])
