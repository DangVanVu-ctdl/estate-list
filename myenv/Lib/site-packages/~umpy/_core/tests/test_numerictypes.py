...
class TestIsSubDType:
    ...
    def test_both_abstract(self):
        assert_(np.issubdtype(floating, np.inexact))
        assert_(not np.issubdtype(np.inexact, floating))

    def test_same(self):
        for cls in (float32, np.int32):
    ...
    def test_subclass(self):
        ...
        for w in self.wrappers:
            assert_(np.issubdtype(w(float32), floating))
            assert_(np.issubdtype(w(float64), floating))

    def test_subclass_backwards(self):
        for w in self.wrappers:
            assert_(not np.issubdtype(floating, w(float32)))
            assert_(not np.issubdtype(floating, w(float64)))

    def test_sibling_class(self):
        for w1, w2 in itertools.product(self.wrappers, repeat=2):
            assert_(not np.issubdtype(w1(float32), w2(float64)))
            assert_(not np.issubdtype(w1(float64), w2(float32)))
    ...
    def test_nondtype_nonscalartype(self):
        ...
        assert not np.issubdtype(float32, 'float64')
        assert not np.issubdtype(float32, 'f8')
        ...
        assert np.issubdtype(float64, "float")
        assert np.issubdtype(float32, "f")
...
class TestIsDType:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (float64, float32), (np.complex128, np.complex64)
        ]
    )
    @pytest.mark.parametrize(
        "dtype_group",
...
class Test_sctype2char:
    ...
    def test_abstract_type(self):
        assert_raises(KeyError, sctype2char, floating)
