...
class TestFactorize:
    ...
    def test_factorize(self, index_or_series_obj, sort):
        ...
        if expected_arr.dtype == float16:
            expected_arr = expected_arr.astype(float32)
    ...
    def test_float64_factorize(self, writable):
        data = np.array([1.0, 1e8, 1.0, 1e-8, 1e8, 1.0], dtype=float64)
        data.setflags(write=writable)
        expected_codes = np.array([0, 1, 0, 2, 1, 0], dtype=np.intp)
        expected_uniques = np.array([1.0, 1e8, 1e-8], dtype=float64)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [1, np.nan, 1, 2],
                np.array([0, 1, 0, 2], dtype=np.dtype("intp")),
                np.array([1, np.nan, 2], dtype=float64),
            ),
        ],
    )
...
class TestUnique:
    ...
    @pytest.mark.parametrize("el_type", [float64, object])
...
class TestIsin:
    ...
    def test_same_nan_is_in(self):
        ...
        # casting to -> float64 -> another float-object somewhere on
    ...
    def test_different_nans(self):
        ...
        # as float64-array:
        result = algos.isin(
            np.asarray(comps, dtype=float64), np.asarray(values, dtype=float64)
        )
        tm.assert_numpy_array_equal(np.array([True]), result)
    ...
    def test_different_nans_as_float64(self):
        ...
        arr = np.array([NAN1, NAN2], dtype=float64)
        lookup1 = np.array([NAN1], dtype=float64)
        result = algos.isin(arr, lookup1)
        expected = np.array([True, True])
        tm.assert_numpy_array_equal(result, expected)

        lookup2 = np.array([NAN2], dtype=float64)
...
class TestValueCounts:
    ...
    @pytest.mark.parametrize("dtype", (float64, object, "M8[ns]"))
...
class TestHashTable:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (ht.Float64HashTable, np.arange(1000, dtype=float64)),
...
class TestRank:
    ...
    def test_basic(self, writable, dtype):
        exp = np.array([1, 2], dtype=float64)
    ...
    @pytest.mark.parametrize("dtype", [float64, np.uint64])
...
class TestMode:
    def test_no_mode(self):
        exp = Series([], dtype=float64, index=Index([], dtype=int))
