...
class TestUfuncGenericLoops:
    ...
    def test_unary_PyUFunc_O_O_method_full(self, ufunc):
        ...
        num_arr = np.array(val, dtype=float64)
...
class TestUfunc:
    ...
    def test_signature_dtype_type(self):
        ...
        float_dtype = type(np.dtype(float64))
    ...
    def test_partial_signature_mismatch(self, casting):
        ...
        res = np.ldexp(float32(1.), np.int_(2), dtype="d")
        assert res.dtype == "d"
        res = np.ldexp(float32(1.), np.int_(2), signature=(None, None, "d"))
        assert res.dtype == "d"
    ...
    def test_partial_signature_mismatch_with_cache(self):
        with pytest.raises(TypeError):
            np.add(float16(1), np.uint64(2), sig=("e", "d", None))
        # Ensure e,d->None is in the dispatching cache (double loop)
        np.add(float16(1), float64(2))
    ...
    def test_cast_safety(self, ufunc):
        ...
        with pytest.raises(TypeError):
            call_ufunc(arr_bs, dtype=float64, casting="equiv")

        # but it is safe cast:
        res = call_ufunc(arr_bs, dtype=float64, casting="safe")
        expected = call_ufunc(arr.astype(float64))  # upcast
        assert_array_equal(expected, res)
    ...
    def test_sum_stability(self):
        a = np.ones(500, dtype=float32)
        assert_almost_equal((a / 10.).sum() - a.size / 10., 0, 4)

        a = np.ones(500, dtype=float64)
        assert_almost_equal((a / 10.).sum() - a.size / 10., 0, 13)

    @pytest.mark.skipif(IS_WASM, reason="fp errors don't work in wasm")
    def test_sum(self):
        for dt in (int, float16, float32, float64, np.longdouble):
    ...
    def test_scalar_reduction(self):
        ...
        assert_(type(np.prod(float32(2.5), axis=0)) is float32)
        assert_(type(np.sum(float32(2.5), axis=0)) is float32)
        assert_(type(np.max(float32(2.5), axis=0)) is float32)
        assert_(type(np.min(float32(2.5), axis=0)) is float32)
    ...
    def test_casting_out_param(self):
        ...
        c = np.ones((200, 100), float64)
        np.add(a, b, out=c)
        assert_equal(c, 2)

        a = np.zeros(65536)
        b = np.zeros(65536, dtype=float32)
    ...
    def test_reduce_identity_depends_on_loop(self):
        ...
        np.add.reduce([], out=out, dtype=float64)
    ...
    def test_reducelike_out_promotes(self):
        ...
        res = np.zeros((), dtype=float32)
        # If `dtype=` is passed, the calculation is forced to float32:
        single_res = np.zeros((), dtype=float32)
        np.multiply.reduce(arr, out=single_res, dtype=float32)
