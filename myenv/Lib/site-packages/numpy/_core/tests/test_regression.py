...
class TestRegression:
    ...
    def test_ndmin_float64(self):
        ...
        assert_equal(np.array(x, dtype=float32, ndmin=2).ndim, 2)
        assert_equal(np.array(x, dtype=float64, ndmin=2).ndim, 2)
    ...
    def test_array_str_64bit(self):
        # Ticket #501
        s = np.array([1, np.nan], dtype=float64)
        ...
        for m in [0, 1, 2]:
            for n in [0, 1, 2]:
                for k in range(3):
                    ...
                    x = np.array([123456789e199], dtype=float64)
                    if IS_PYPY:
                        x.resize((m, 0), refcheck=False)
                    else:
                        x.resize((m, 0))
                    y = np.array([123456789e199], dtype=float64)
                    if IS_PYPY:
    ...
    def test_ticket_1538(self):
        ...
        for name in 'eps epsneg max min resolution tiny'.split():
            assert_equal(type(getattr(x, name)), float32,
                         err_msg=name)
    ...
    def test_array_scalar_contiguous(self):
        ...
        assert_(np.array(float32(1.0)).flags.c_contiguous)
        assert_(np.array(float32(1.0)).flags.f_contiguous)
    ...
    def test_logspace_base_does_not_determine_dtype(self):
        ...
        start = np.array([0, 2], dtype=float16)
        stop = np.array([2, 0], dtype=float16)
        out = np.logspace(start, stop, num=5, axis=1, dtype=float32)
        ...
        out2 = np.logspace(start, stop, num=5, axis=1, dtype=float32,
                           base=np.array([10.0]))
