...
def solve_circulant(c, b, singular='raise', tol=None,
    ...
    tol : float, optional
        ...
        near singular. If not given, `tol` is set to::
            ...
            tol = abs_eigs.max() * abs_eigs.size * np.finfo(float64).eps
    ...
    if tol is None:
        ...
        tol = abs_fc.max(axis=-1) * nc * np.finfo(float64).eps
        if tol.shape != ():
...
def det(a, overwrite_a=False, check_finite=True):
    ...
    if min(*a1.shape) == 0:
        dtyp = float64 if a1.dtype.char not in 'FD' else np.complex128
        if a1.ndim == 2:
            return dtyp(1.0)
        else:
    ...
    if a1.shape[-2:] == (1, 1):
        ...
        if max(*a1.shape) > 1:
            ...
            return (float64(a1.item()) if a1.dtype.char in 'fd' else
    ...
    if a1.ndim == 2:
        ...
        return (float64(det) if np.isrealobj(det) else np.complex128(det))

    # loop over the stacked array, and avoid overflows for single precision
    # Cf. np.linalg.det(np.diag([1e+38, 1e+38]).astype(float32))
...
def _validate_args_for_toeplitz_ops(c_or_cr, b, check_finite, keep_b_shape,
    ...
    dtype = np.complex128 if is_cmplx else float64
